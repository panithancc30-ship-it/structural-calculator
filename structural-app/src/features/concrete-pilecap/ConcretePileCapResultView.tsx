...
import { FootingBarPopover } from '@/components/concrete/footing/FootingPopover'
import { NumberInput } from '@/components/concrete/form/Fields'
...
import {
  ...
  planTitle,
  type PileCapPick,
} from '@/components/concrete/pilecap/pileCapDrawingModel'
                ...
                <tr key={i} className={status}>
                  ...
                  <td className="num">
                    <NumberInput
                      value={offsets[i].dx}
                      step={1}
                      ariaLabel={`เข็มต้นที่ ${i + 1} เยื้อง dx`}
                      onChange={(dx) => setOffset(i, { dx })}
                    />
                  </td>
                  <td className="num">
                    <NumberInput
                      value={offsets[i].dy}
                      step={1}
                      ariaLabel={`เข็มต้นที่ ${i + 1} เยื้อง dy`}
                      onChange={(dy) => setOffset(i, { dy })}
                    />
                  </td>
                  <td className="num">
                    <b>{fmt(R / 1000, 2)}</b>
...
export function ConcretePileCapResultView() {
  ...
  const analysis = useMemo(
    () => (design ? analyzePileCap(input, design.arrangement, design.t, layout) : null),
    [design, input, layout],
  )
  const suggested = useMemo(
    () => (design ? autoPileCapLayout(input, design.arrangement, design.t) : null),
    [design, input],
  )
  ...
  const eccentric =
    analysis &&
    Math.hypot(
      analysis.loads.piles.column.x - analysis.loads.service.xg,
      analysis.loads.piles.column.y - analysis.loads.service.yg,
    ) > 0.05
        ...
        <>
          ...
          <div className="footing-grid">
            <section className="card section-card">
              ...
              <p className="hint">
                คลิกที่เหล็กหรือรายการเหล็กใต้รูป เพื่อเปลี่ยนขนาด / จำนวน — กากบาทคือตำแหน่งเข็มตามแบบ เมื่อเข็มเยื้อง
              </p>
              <div className="toolbar">
