...
import type { BarDir, FootingInput } from '../../domain/footing/types';
...
export function Leaders({
  leaders, u, size, subSize, selectedDir, pick,
}: {
  leaders: Leader[];
  u: number;
  size: number;
  ...
  return (
    <>
      {leaders.map((l) => {
        const color = selectedDir === l.dir ? HIGHLIGHT : INK;
        const { className, ...handlers } = pick(l.dir) as { className?: string };
        return (
...
export function Frame({
  view, sizeMm, physicalSize, interactive, label, onBackgroundClick, children,
}: {
  ...
  return (
    <svg
      className={`section-drawing footing-drawing${interactive ? ' interactive' : ''}`}
