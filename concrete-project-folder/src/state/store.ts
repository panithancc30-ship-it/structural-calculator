...
import { autoLayout } from '../domain/design/designBeam';
import { faceArea } from '../domain/layoutOps';
...
export const useStore = create<AppState>()(
  persist(
    ...
    }),
    {
      name: 'rc-beam-wsd',
      version: 1,
      partialize: (s) => ({ input: s.input, layouts: s.layouts, edited: s.edited }),
      merge: (persisted, current) => {
        const result = parseProject({ ...(asObj(persisted) ?? {}), app: 'rc-beam-wsd' });
        return typeof result === 'string'
          ? current
          : { ...current, input: result.input, layouts: result.layouts, edited: result.edited };
      },
    },
  ),
);
