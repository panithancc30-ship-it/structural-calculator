...
import { ACI318_WSD_PILECAP as K } from '../domain/codes/aci318Wsd';
import type { FootingLayout } from '../domain/footing/types';
import { autoPileCapLayout, pileCapDesign } from '../domain/pilecap/designPileCap';
import type { PileCapInput, PileOffset } from '../domain/pilecap/types';
import { AUTO_MODES, PILE_SHAPES, validatePileCapInput } from '../domain/pilecap/validate';
import { isBarName } from '../domain/rebar';
...
export const usePileCapStore = create<PileCapState>()(
  persist(
    ...
    }),
    {
      name: 'rc-pilecap-wsd',
      version: 1,
      partialize: (s) => ({ input: s.input, layout: s.layout, edited: s.edited }),
      merge: (persisted, current) => {
        const result = parsePileCapProject({ ...(asObj(persisted) ?? {}), app: 'rc-pilecap-wsd' });
        return typeof result === 'string' ? current : { ...current, input: result.input, layout: result.layout, edited: result.edited };
      },
    },
  ),
);
